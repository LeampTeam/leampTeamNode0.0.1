            ...
            .exec((err, productos) => {
                ...
                Categoria.find({
                    ...
                    .exec((err, cat) => {
                        ...
                        Producto.find({}, ).populate('fragancia').populate('tamano').populate('presentacion')
                            .exec((err, productos) => {
...
function getproductoById(req, res) {
    ...
    req.session.total = total.toFixed(2)
    Producto.findById(id, ).populate('categoria').populate('fragancia').populate('tamano').populate('presentacion')
                ...
                .exec((err, productos) => {
                    ...
                    Categoria.find({
                        ...
                        .exec((err, cat) => {
                            ...
                            return res.render('frontEnd/articuloDetalle', {
                                produ,
                                progroupFrag,
                                progroupTam,
                                progroupLitr,
    ...
    if (req.session.total == 0) {
        Categoria.find({
            ...
            .exec((err, cat) => {
                ...
                return res.render('frontEnd/carro-compra', {
                    mensaje: "El carro de compras esta vacio",
                    cateVista,
                    total,
                    verChango
                })
        ...
        Categoria.find({
            ...
            .exec((err, cat) => {
                ...
                return res.render('frontEnd/carroCompra', {
                    verChango,
                    total,
                    cateVista
                })
...
async function enviarMail(req, res) {
    ...
    pedido.estado = await Estado.findOne({
        codigo: 1
    }).exec()
...
function getProduct(req, res) {
    ...
    if (busqueda.cate && busqueda.fraga && busqueda.prese) {
        Producto.find({
                categoria: busqueda.cate,
                fragancia: busqueda.fraga,
                presentacion: busqueda.prese
            },
        ...
        Producto.find({
            ...
            function (error, productos) {
             ...
             } else if (busqueda.cate && busqueda.prese && busqueda.tama) {
                 Producto.find({
                         categoria: busqueda.cate,
                         presentacion: busqueda.prese,
                         tamano: busqueda.tama
    ...
    } else if (busqueda.tama && busqueda.fraga) {
        Producto.find({
                tamano: busqueda.tama,
                fragancia: busqueda.fraga
            },
    ...
    } else if (busqueda.tama && busqueda.prese) {
        Producto.find({
                tamano: busqueda.tama,
                presentacion: busqueda.prese
            },
...
function buscarProductos(req, res) {
    ...
    Producto.find({
        $or: [{
            name: new RegExp(params.busqueda, "i")
        }, {
            descripcion: new RegExp(params.busqueda, "i")
        }],
        eliminado: {
            $ne: true
        }
        ...
        if (productos.length == 0) {
            Categoria.find({
                ...
                .exec((err, cat) => {
                    ...
                    res.render('frontEnd/carro-compra', {
                        mensaje: 'No se encontraron resultados para esta busqueda',
                        cateVista,
                        verChango: req.session.productocomprado,
                        total: req.session.total
                    })
